...
class PrayerTimesNotifier extends ChangeNotifier {
  ...
  Future<void> _initializeLocationAndPrayerTimes() async {
    ...
    if (settings['fajr'] != null) {
      _prayerService.prayerSettings.fajr = settings['fajr'] ?? '0';
      _prayerService.prayerSettings.tulu = settings['tulu'] ?? '0';
      _prayerService.prayerSettings.dhuhr = settings['dhuhr'] ?? '0';
      _prayerService.prayerSettings.asr = settings['asr'] ?? '0';
      _prayerService.prayerSettings.magrib = settings['magrib'] ?? '0';
      _prayerService.prayerSettings.isha = settings['isha'] ?? '0';
    }
  ...
  String getPrayerTime(Prayer prayer) {
    return _prayerService.getPrayerTime(prayer, getAdditionalTime(prayer));
  }
